...
export class CategoriesService {
  ...
  private async loadCategories() {
    try {
      ...
      categorySnapshot.forEach((cat) =>
        categories.push(cat.data() as Category)
      )
  ...
  async editCategory(cat: Category) {
    ...
    if (index !== -1) {
      ...
      const q = query(categoryRef, where('id', '==', cat.id))
      const querySnapshot = await getDocs(q)
      if (!querySnapshot.empty) {
        const docToUpdate = querySnapshot.docs[0]
        await updateDoc(doc(this.firestore, 'Category', docToUpdate.id), {
          ...cat,
        })
        //update local state
  ...
  async addCategory(cat: Category) {
    try {
      ...
      await addDoc(categoryRef, {
        ...cat,
        id: String(this.categoriesSubject.value.length + 1),
      })
